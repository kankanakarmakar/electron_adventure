    ...
    <div className="relative h-screen bg-background overflow-hidden flex items-center justify-center">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center p-4 max-w-6xl mx-auto">
        ...
        <h1 className="text-4xl md:text-5xl font-display font-black tracking-tight text-center mb-6">
          Electronics
          <span className="block md:inline text-transparent bg-clip-text bg-gradient-to-r from-primary via-cosmic-purple to-secondary">Universe</span>
        </h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full max-w-5xl">
          {[
            ...
            <div
              key={m.key}
              className="group relative rounded-xl bg-card/30 backdrop-blur-md border border-border/40 p-5 text-center transition-all cursor-pointer overflow-hidden will-change-transform hover:shadow-[0_10px_30px_hsl(var(--primary)/0.15)]"
              onClick={() => {
            ...
            >
              ...
              <span className="pointer-events-none absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity">
                <span className="absolute inset-0 rounded-2xl" style={{
                  background: 'conic-gradient(from 180deg at 50% 50%, hsl(var(--primary)/0.12), hsl(var(--secondary)/0.12), hsl(var(--primary)/0.12))'
                  }} />
              </span>

              {/* Glow overlay */}
              <div className="absolute -top-24 left-1/2 -translate-x-1/2 w-64 h-64 bg-primary/5 blur-xl rounded-full opacity-0 group-hover:opacity-100 transition-opacity" />
